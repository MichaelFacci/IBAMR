...
struct TetherData
{
    ...
    TetherData(Pointer<Database> input_db)
        : c1_s(input_db->getDouble("C1_S")),
          kappa_s_surface(input_db->getDouble("KAPPA")),
          eta_s_surface(input_db->getDouble("ETA"))
    {
    }
};
                      ...
                      void* ctx)
{
    ...
    {
        F(d) =  tether_data->eta_s_surface * (upper_drift_velocity - U[d]);
    }
    return;
                      ...
                      void* ctx)
{
    ...
    const std::vector<double>& U = *var_data[0];
    for (unsigned int d = 0; d < NDIM; ++d)
        F(d) =  tether_data->eta_s_surface * (lower_drift_velocity - U[d]);  
    return;
} // tether_force_function
} // namespace ModelData
...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        for (unsigned int i = 0; i <= n_elem_gen; i++){
            mesh_lower.add_point(libMesh::Point(left_end + ds * i,-separation/2.0),node_id++);
        }
        ...
        IIMethod::LagSurfaceForceFcnData  tether_force_lower_data(tether_force_function_lower, sys_data,tether_data_ptr);
        IIMethod::LagSurfaceForceFcnData  tether_force_upper_data(tether_force_function_upper, sys_data,tether_data_ptr);
        ...
        std::unique_ptr<ExodusII_IO> lower_exodus_io(uses_exodus ? new ExodusII_IO(mesh_lower) : NULL);
        std::unique_ptr<ExodusII_IO> upper_exodus_io(uses_exodus ? new ExodusII_IO(mesh_upper) : NULL);
