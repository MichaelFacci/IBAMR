...
static bool use_phong = false;
static bool use_normalized_metric = false;
static bool use_angle_metric = false;
void
...
double compute_perpendicularity_metric_simple(
...
{
    // --- 1. Get Systems ---
    EquationSystems* equation_systems = ib_method_ops->getFEDataManager(part)->getEquationSystems();
    
    if (!equation_systems->has_system("velocity [[du]] jump system") ||
        !equation_systems->has_system("[[p]] system"))
    {
        return 0.0;
    }
    
    System& vel_x_sys = equation_systems->get_system("velocity [[du]] jump system");
    System& vel_y_sys = equation_systems->get_system("velocity [[dv]] jump system");
    System& vel_z_sys = equation_systems->get_system("velocity [[dw]] jump system");
    System& p_sys = equation_systems->get_system("[[p]] system");
    
    System* normal_sys = (use_smoothed && !use_phong && equation_systems->has_system("smoothed normal system")) ? 
                         &equation_systems->get_system("smoothed normal system") : nullptr;

    System* X_system = nullptr;
    NumericVector<double>* X_vec = nullptr;
    if (use_phong) {
        X_system = &equation_systems->get_system("coordinates system");
        X_vec = ib_method_ops->getMeshCoordinatesNumeric(true, "current", part);
    }
    
    // --- 2. Localize Vectors ---
    NumericVector<double>& vel_x_vec = *vel_x_sys.current_local_solution;
    NumericVector<double>& vel_y_vec = *vel_y_sys.current_local_solution;
    NumericVector<double>& vel_z_vec = *vel_z_sys.current_local_solution;
    NumericVector<double>& p_vec     = *p_sys.current_local_solution;
    NumericVector<double>* n_vec     = normal_sys ? normal_sys->current_local_solution.get() : nullptr;

    // --- 3. Initialize Stats ---
    double perpendicularity_sum = 0.0;
    double total_weight = 0.0;
    
    double angle_sum = 0.0;
    double total_angle_weight = 0.0;
    double min_angle = 180.0;
    double max_angle = 0.0;
    bool   angle_data_found = false; 

    double norm_dot_sum = 0.0;
    double norm_dot_weight = 0.0;
    double min_norm_dot = 1.0; 
    double max_norm_dot = 0.0; 

    double min_dot_product = 1.0e30; // Un-normalized metric min
    double max_dot_product = 0.0;    // Un-normalized metric max

    // --- 4. Element Loop ---
    MeshBase::const_element_iterator el = mesh_ref.active_local_elements_begin();
    const MeshBase::const_element_iterator end_el = mesh_ref.active_local_elements_end();
    
    // Only proceed if we have elements
    if (el != end_el)
    {
        const Elem* first_elem = *el;
        const unsigned int elem_dim = first_elem->dim();
        
        FEType fe_type = vel_x_sys.get_dof_map().variable_type(0);
        std::unique_ptr<FEBase> fe = FEBase::build(elem_dim, fe_type);
        std::unique_ptr<QBase> qrule = QBase::build(QGAUSS, elem_dim, SECOND);
        ...
        for (; el != end_el; ++el)
        {
            ...
            std::vector<dof_id_type> vel_x_dofs, vel_y_dofs, vel_z_dofs, p_dofs;
            vel_x_sys.get_dof_map().dof_indices(elem, vel_x_dofs);
            vel_y_sys.get_dof_map().dof_indices(elem, vel_y_dofs);
            vel_z_sys.get_dof_map().dof_indices(elem, vel_z_dofs);
            p_sys.get_dof_map().dof_indices(elem, p_dofs);
            
            // --- A. Element Normal Strategy ---
            VectorValue<double> n_elem_const;
            boost::multi_array<double, 2> x_node;

            if (use_phong) {
                std::vector<std::vector<dof_id_type> > X_dof_indices(NDIM);
                for (unsigned int d = 0; d < NDIM; ++d) X_system->get_dof_map().dof_indices(elem, X_dof_indices[d], d);
                get_values_for_interpolation(x_node, *X_vec, X_dof_indices);
            }
            else if (use_smoothed && n_vec) {
                std::vector<dof_id_type> n_x_dofs, n_y_dofs, n_z_dofs;
                normal_sys->get_dof_map().dof_indices(elem, n_x_dofs, 0);
                normal_sys->get_dof_map().dof_indices(elem, n_y_dofs, 1);
                normal_sys->get_dof_map().dof_indices(elem, n_z_dofs, 2);
                
                n_elem_const(0) = (*n_vec)(n_x_dofs[0]);
                n_elem_const(1) = (*n_vec)(n_y_dofs[0]);
                n_elem_const(2) = (*n_vec)(n_z_dofs[0]);
                if (n_elem_const.norm() > 1e-12) n_elem_const = n_elem_const.unit();
            }
            else {
                // Flat normal fallback
                libMesh::Point p0 = elem->point(0);
                libMesh::Point p1 = elem->point(1);
                libMesh::Point p2 = elem->point(2);
                libMesh::Point v1 = p1 - p0;
                libMesh::Point v2 = p2 - p0;
                VectorValue<double> n_cross = v1.cross(v2);
                n_elem_const(0) = n_cross(0);
                n_elem_const(1) = n_cross(1);
                n_elem_const(2) = n_cross(2);
                if (n_elem_const.norm() > 1e-12) n_elem_const = n_elem_const.unit();
            }
            ...
            {
                // --- B. QP Normal ---
                VectorValue<double> n_qp_vec;
                if (use_phong) {
                    n_qp_vec = ib_method_ops->evaluateNormalVectors(true, qp, true, const_cast<Elem*>(elem), x_node, phi, dphi_dxi, part);
                    if (n_qp_vec.norm() > 1e-12) n_qp_vec = n_qp_vec.unit();
                } else {
                    n_qp_vec = n_elem_const;
                }

                // --- C. Interpolate Jumps ---
                double jump_du_x=0, jump_du_y=0, jump_du_z=0, jump_p=0;
                for (size_t i=0; i<phi.size(); ++i) {
                    jump_du_x += phi[i][qp] * vel_x_vec(vel_x_dofs[i]);
                    jump_du_y += phi[i][qp] * vel_y_vec(vel_y_dofs[i]);
                    jump_du_z += phi[i][qp] * vel_z_vec(vel_z_dofs[i]);
                    jump_p    += phi[i][qp] * p_vec(p_dofs[i]);
                }

                // --- D. Metric Vectors ---
                // Vec A = -[[du/dn]]
                VectorValue<double> vec_A(-jump_du_x, -jump_du_y, -jump_du_z);
                // Vec B = [[p]] * n
                VectorValue<double> vec_B = n_qp_vec * jump_p;
                
                double len_A = vec_A.norm();
                double len_B = vec_B.norm();

                // 1. Standard Perpendicularity Metric (Weighted Dot Product)
                double dot_raw = vec_A * vec_B;
                double dot_val_for_metric = 0.0;
                
                if (use_normalized_metric) {
                    VectorValue<double> A_hat = (len_A > 1e-14) ? vec_A/len_A : vec_A*0.0;
                    VectorValue<double> B_hat = (len_B > 1e-14) ? vec_B/len_B : vec_B*0.0;
                    dot_val_for_metric = A_hat * B_hat;
                } else {
                    dot_val_for_metric = dot_raw;
                }
                double abs_dot = std::abs(dot_val_for_metric);
                
                perpendicularity_sum += abs_dot * JxW[qp];
                total_weight += JxW[qp];
                
                if (abs_dot < min_dot_product) min_dot_product = abs_dot;
                if (abs_dot > max_dot_product) max_dot_product = abs_dot;

                // 2. Angle & Normalized Dot Statistics
                // Only compute if vectors are large enough to be meaningful
                if (len_A > 1e-12 && len_B > 1e-12) {
                    double cos_theta = dot_raw / (len_A * len_B);
                    ...
                    angle_sum += angle_deg * JxW[qp];
                    total_angle_weight += JxW[qp];
                    
                    // Logic to init/update min/max
                    if (!angle_data_found) {
                        min_angle = angle_deg;
                        max_angle = angle_deg;
                        angle_data_found = true;
                    } else {
                        if (angle_deg < min_angle) min_angle = angle_deg;
                        if (angle_deg > max_angle) max_angle = angle_deg;
                    }

                    // Track Normalized Dot Product Magnitude (independent of metric flag)
                    double norm_dot_mag = std::abs(cos_theta);
                    norm_dot_sum += norm_dot_mag * JxW[qp];
                    norm_dot_weight += JxW[qp];
                    
                    if (norm_dot_mag < min_norm_dot) min_norm_dot = norm_dot_mag;
                    if (norm_dot_mag > max_norm_dot) max_norm_dot = norm_dot_mag;
                }
            }
        }
    }
    ...
    // --- 5. MPI Reductions ---
    // Standard metric reductions
    SAMRAI_MPI::sumReduction(&perpendicularity_sum, 1);
    SAMRAI_MPI::sumReduction(&total_weight, 1);
    SAMRAI_MPI::minReduction(&min_dot_product, 1);
    SAMRAI_MPI::maxReduction(&max_dot_product, 1);
    
    // Angle/Normalized Stats reductions
    if (use_angle_metric) {
        SAMRAI_MPI::sumReduction(&angle_sum, 1);
        SAMRAI_MPI::sumReduction(&total_angle_weight, 1);
        
        // Determine if ANY processor found valid data
        double local_found = angle_data_found ? 1.0 : 0.0;
        double global_found = local_found; 
        SAMRAI_MPI::maxReduction(&global_found, 1); 

        if (global_found > 0.0) {
            // Safe to reduce min/max
            // If a proc didn't find data, reset its local Min/Max to extreme opposites
            // so it doesn't pollute the reduction (e.g. if I init to 180/0 and send that, it might override real data)
            // However, SAMRAI_MPI reductions are simple wrappers. 
            // A safer bet: if !angle_data_found, set min=1e9, max=-1e9 before calling reduction?
            // Actually, we initialized min=180, max=0.
            // If we have valid data, min will be <= 180, max >= 0.
            // BUT: standard minReduction will take the absolute minimum.
            // If we didn't find data, we should contribute "Neutral" values.
            if (!angle_data_found) {
                min_angle = 1000.0; // Larger than possible 180
                max_angle = -1000.0; // Smaller than possible 0
                min_norm_dot = 10.0;
                max_norm_dot = -10.0;
            }
            
            SAMRAI_MPI::minReduction(&min_angle, 1);
            SAMRAI_MPI::maxReduction(&max_angle, 1);
            
            SAMRAI_MPI::sumReduction(&norm_dot_sum, 1);
            SAMRAI_MPI::sumReduction(&norm_dot_weight, 1);
            SAMRAI_MPI::minReduction(&min_norm_dot, 1);
            SAMRAI_MPI::maxReduction(&max_norm_dot, 1);
        } else {
            // No valid vectors found anywhere
            min_angle = 0.0; max_angle = 0.0; 
            min_norm_dot = 0.0; max_norm_dot = 0.0;
        }
    }

    // Averages
    double avg_metric = (total_weight > 0) ? perpendicularity_sum / total_weight : 0.0;
    double avg_angle = (total_angle_weight > 0) ? angle_sum / total_angle_weight : 0.0;
    double avg_norm_dot = (norm_dot_weight > 0) ? norm_dot_sum / norm_dot_weight : 0.0;

    // --- 6. Log Output (Rank 0) ---
    if (SAMRAI_MPI::getRank() == 0)
    {
        std::ofstream log_file;
        const std::string filename = "metric_log";
        bool write_header = false;

        std::ifstream fcheck(filename);
        if (fcheck.peek() == std::ifstream::traits_type::eof() || !fcheck.good()) write_header = true;
        fcheck.close();

        log_file.open(filename, std::ios_base::app);
        if (write_header) {
            log_file << "time,max_angle,min_angle,avg_angle,norm_dot_min,norm_dot_max,norm_dot_avg\n";
        }
        
        log_file << std::setprecision(8) << current_time << ","
                 << max_angle << ","
                 << min_angle << ","
                 << avg_angle << ","
                 << min_norm_dot << ","
...
static ofstream max_displacement_stream,flow_rate_stream, perpendicularity_stream;
...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        use_smoothed = input_db->getBool("USE_SMOOTHED_NORMALS");
        use_phong = input_db->getBool("USE_PHONG_NORMALS");
        use_normalized_metric = input_db->getBool("USE_NORMALIZED_METRIC");
        use_angle_metric = input_db->getBoolWithDefault("USE_ANGLE_METRIC", false);
	...
	if(use_feedback_forcer){
        ...
        if (SAMRAI_MPI::getRank() == 0)
        {
            ...
            perpendicularity_stream.open("perpendicularity.dat", ios_base::out | ios_base::trunc);
            perpendicularity_stream << "# Time  Perpendicularity_Metric" << endl;
        }
        // Main time step loop.
        ...
        while (!MathUtilities<double>::equalEps(loop_time, loop_time_end) && time_integrator->stepsRemaining())
        {
            ...
            {
                double perp_metric = compute_perpendicularity_metric_simple(ib_method_ops, 0, cylinder_mesh_thin, loop_time);
                
                //pout << "\n===========================================\n";
                //pout << "Perpendicularity Metric (Equation 15):\n";
                //pout << "  Value: " << perp_metric << "\n";
                //pout << "===========================================\n\n";
                
                if (SAMRAI_MPI::getRank() == 0)
                {
                    perpendicularity_stream << loop_time << " " << perp_metric << endl;
                    perpendicularity_stream.flush();
                }
            }
