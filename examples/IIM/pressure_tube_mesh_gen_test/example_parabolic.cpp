...
main(int argc, char* argv[])
{
	...
	// Get various standard options set in the input file.
        ...
        */
        ...
        cylinder_mesh_thin.reserve_elem(2* NRi_elem * NXi_elem);
        ...
        for (unsigned int j = 0; j <= NXi_elem - 1; j++)
        {
            ...
            {
                const unsigned int n0 = idx(NRi_elem, i,     j);
                const unsigned int n1 = idx(NRi_elem, i + 1, j);
                const unsigned int n2 = idx(NRi_elem, i + 1, j + 1);
                const unsigned int n3 = idx(NRi_elem, i,     j + 1);

                // First triangle (n0,n1,n2)
                {
                    Elem* tri1 = cylinder_mesh_thin.add_elem(new Tri3);
                    tri1->set_node(0) = cylinder_mesh_thin.node_ptr(n0);
                    tri1->set_node(1) = cylinder_mesh_thin.node_ptr(n1);
                    tri1->set_node(2) = cylinder_mesh_thin.node_ptr(n2);
                }

                // Second triangle (n0,n2,n3)
                {
                    Elem* tri2 = cylinder_mesh_thin.add_elem(new Tri3);
                    tri2->set_node(0) = cylinder_mesh_thin.node_ptr(n0);
                    tri2->set_node(1) = cylinder_mesh_thin.node_ptr(n2);
                    tri2->set_node(2) = cylinder_mesh_thin.node_ptr(n3);
                }
            }
        }
        ...
        for (unsigned int j = 0; j <= NXi_elem - 1; j++)
        {
            const unsigned int n0 = idx(NRi_elem, NRi_elem - 1, j);
            const unsigned int n1 = idx(NRi_elem, 0,            j);
            const unsigned int n2 = idx(NRi_elem, 0,        j + 1);
            const unsigned int n3 = idx(NRi_elem, NRi_elem - 1, j + 1);
