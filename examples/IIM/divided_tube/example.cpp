...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        const int num_elems = (int)2.0/ds; //Height of the domain is 2, so number of elems in a straight line is
                                        // simply b-a/dx
        std::cout <<"num elems is: "<<num_elems;
        //MeshTools::Generation::build_line(solid_mesh, num_elems, -0.9, 0.9, Utility::string_to_enum<ElemType>(elem_type));
        //MeshTools::Generation::build_square(solid_mesh, 1, num_elems, 2.5-ds/4, 2.5+ds/4,0.0, 2.0, Utility::string_to_enum<ElemType>(elem_type));
        double theta = M_PI/4; //rotate 90 degrees, perpendicular to flow
        //solid_mesh.set_mesh_dimension(1);
        libMesh::GmshIO gmsh_io(solid_mesh);
        gmsh_io.read("straightline.msh");   

        //rotateMesh(solid_mesh, theta);

        /*
        for (int k = 0; k < num_elems + 1; ++k)
        {   
            double y = k*ds;
            solid_mesh.add_point(libMesh::Point(2.5,y));
        }
    ...
    */
    ...
    //testing to see the nodes of the mesh
        ...
        for (const auto & elem : mesh.active_local_element_ptr_range())
        {
            ...
            std::cout << "\n\ncurrent element index: " <<current_elem_index <<"\n";
            //Get the measure of the current element
            double measure = elem->volume(); 
            std::cout << "elem measure: " <<measure <<"\n";
            //Get reference to the nodes
            const unsigned int n_nodes = elem->n_nodes();
            std::cout << "elem nodes owned: " <<n_nodes <<"\n";
            ...
            for (unsigned int node_idx = 0; node_idx < NDIM; ++node_idx)
                {
                    std::cout << "node local index: " <<node_idx <<"\n";
                    ...
                    if(node_idx == 1 && NDIM == 2){
                        std::cout << "  first x_node point: " <<x_node[0][0]<<", "<<x_node[0][1]<<", "<<x_node[0][2]<<"\n";
                        std::cout << "  second x_node point: " <<x_node[1][0]<<", "<<x_node[1][1]<<", "<<x_node[1][2]<<"\n";
                    }
                    if(node_idx == 2 && NDIM == 3){
                        std::cout << "  first x_node point: " <<x_node[0][0]<<", "<<x_node[0][1]<<", "<<x_node[0][2]<<"\n";
                        std::cout << "  second x_node point: " <<x_node[1][0]<<", "<<x_node[1][1]<<", "<<x_node[1][2]<<"\n";
                        std::cout << "  third x_node point: " <<x_node[2][0]<<", "<<x_node[2][1]<<", "<<x_node[2][2]<<"\n";
                    }

                }
        }
        libMesh::ExodusII_IO exodus_see(solid_mesh);
    
        if(NDIM == 2){
            exodus_see.write("meshOutput2d.e");
        }
