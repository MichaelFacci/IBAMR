...
IIMethod::computeFluidTraction(const double data_time, unsigned int part)
{
    ...
    boost::multi_array<double, 2> x_node, X_node, WSS_in_node, WSS_out_node, n_qp_node, Normal_node, Smoothed_Normal_node;
...
IIMethod::getFromInput(Pointer<Database> db, bool /*is_from_restart*/)
{
    ...
    if (db->isDouble("interp_point_density"))
     ...
     if (db->isDouble("normal_stabilization_eps"))
    {
        d_normal_stabilization_eps = db->getDouble("normal_stabilization_eps");
    }
